...
namespace Microsoft.Partner.SmartOffice.Services.PartnerCenter.AuditRecords
{
    ...
    public class AuditRecordCollectionOperations : IAuditRecordCollectionOperations
    {
        ...
        public async Task<SeekBasedResourceCollection<AuditRecord>> QueryAsync(string id, DateTime startDate, DateTime? endDate = null, CancellationToken cancellationToken = default(CancellationToken))

        {

            string filter = $"\"Field\":\"CustomerId\",\"Value\":\"{id}\",\"Operator\":\"equals\"";

            filter = "{" + filter + "}";

            var encoded = Uri.EscapeUriString(filter);

            return await client.GetAsync<SeekBasedResourceCollection<AuditRecord>>(

                new Uri($"/v1/auditrecords?startDate={startDate.ToString(CultureInfo.InvariantCulture)}&filter={encoded}", UriKind.Relative),
