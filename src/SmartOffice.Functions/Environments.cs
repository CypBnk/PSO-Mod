...
namespace Microsoft.Partner.SmartOffice.Functions
{
    ...
    public static class Environments
    {
            ...
            try
            {
                ...
                else
                {
                    auditRecords = await GetAuditRecordsAsyc(
                        ...
                        DateTime.UtcNow, environment.Id).ConfigureAwait(false);
        ...
        public static async Task PullEnvironmentsAsync(
            [TimerTrigger("0 0 9 * * *", RunOnStartup = true)]TimerInfo timerInfo,
            [DataRepository(
            ...
            DateTime endDate, string id)
        {
            ...
            try
            {
                ...
                foreach (DateTime date in ChunkDate(startDate, endDate, 30))
                {
                    ...
                    seekAuditRecords = await client.AuditRecords.QueryAsync(id, date).ConfigureAwait(false);
